          ...
          app.request().get('/search/all').query({
            ...
            if (randomKeyLocal === randomKey) {
              ...
              if (data.code == 0 && data.data) {
                ...
                if (_data.movieEntities.length > 0 || _data.cinemaEntities.length > 0) {
                  hasResults = true;
                  _data.cinemaEntities.map(function (item) {
                    item.movieId = _this2.options.movieId;
                  });
                }
              }
              var setData = function setData() {
                _this2.setData({
                  keyword: keyword,
                  // results: data.data,
                  movieEntities: _this2.options.type == 2 ? [] : _this2.groupMoviesByDate(_data.movieEntities),
